...
function get_extended_desc($desc, $param = '')
{
  ...
  if (preg_match('#\[slick-slider(.*?)\]#i', $desc)) {
    // Chèn CSS và JS cho slider
    add_event_handler('loc_end_page_header', 'add_slick_slider_css', EVENT_HANDLER_PRIORITY_NEUTRAL, 3);
    add_event_handler('loc_end_page_tail', 'add_slick_slider_js', EVENT_HANDLER_PRIORITY_NEUTRAL, 3);
  }
  ...
  $generic_pattern = '#\[(cat=|img|photo|random|slider|login-link|slick-slider)([^\]]*)\]#i';
...
{
  echo '<script async src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.7.1/jquery.min.js" integrity="sha512-v2CJ7UaYy4JwqLDIrZUI/4hqeoQieOmAZNXBeQyjo21dadnwR+8ZaIJVT8EE2iyI61OV8e6M8PP2/4hpQINQ/g==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>';
  echo '<script  async src="https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.9.0/slick.min.js" integrity="sha512-HGOnQO9+SP1V92SrtZfjqxxtLmVzqZpjFFekvzZVWoiASSQgSr4cw9Kqd2+l8Llp4Gm0G8GIFJ4ddwZilcdb8A==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>';
  echo '<script async type="text/javascript" src="./plugins/ExtendedDescription/template/assets/slick-slider.js"></script>';
}
